...
// Script to broadcast coupons to blockchain 
...
let db = new sqlite3.Database('main.db', (err) => {
 ...
 //--------------------------------------------------------------
// Sukuk smart contracts configuration 
//--------------------------------------------------------------
...
//--------------------------------------------------------------
// Broadcasting coupon to blockchain 
//--------------------------------------------------------------
db.all('SELECT * FROM temp_coupons WHERE inBlockchain IS NULL', (err,rows)=>{
    if(rows.length > 0)
    {
    console.log("Rows number ", rows.length)
    const forLoop = async _ => {
      for (let index = 0; index <rows.length; index++) {
        console.log("Coupon Id: ", rows[index].Id)
    ...
    forLoop().then(function(error,result){
        console.log('All Coupons are Broadcasted')
    ...
    {
      console.log('[Coupons] ::: No coupon to broadcast')
      process.exit(0)
    }
    if (err)
    {
